...
--- FAIL: TestBuildAuditNeedlesWidthCrossing
    replaceaudit_test.go:63: needle[6] = "gitmap-v20", want "gitmap-v20"
--- FAIL: TestRemoteSlugRegex
    replaceversionparse_test.go:57: regex "gitmap-v20" -> base="gitmap" num="13", want base="gitmap" num="12"
```
...
"gitmap-v20": {true, "gitmap", "12"},   // ← "12" was the captured num for v12
```
...
want := []string{
    ...
    "gitmap-v20", "gitmap/v12",  // ← only "gitmap-v20" got bumped
}
```

When fix-repo rewrites `gitmap-v20` → `gitmap-v20`, the sibling `"12"` / `12` remains, leaving an internally inconsistent test that fails on the next `go test` run.
...
3. The audit gives a precise diagnostic (`paired-literal desync at file:line: 'gitmap-v20' followed by sibling '12'`) instead of a downstream test failure.
