...
// historical desync where fixtures hard-coded `gitmap-v18` next to
...
func TestLineContainsAny(t *testing.T) {
	needles := [][]byte{[]byte("gitmap-v18"), []byte("gitmap/v9")}
	if !lineContainsAny([]byte("import gitmap-v18/foo"), needles) {
...
func TestScanAuditFileFormatting(t *testing.T) {
	...
	want2 := fmt.Sprintf(constants.MsgReplaceAuditMatch, path, 2, "see gitmap-v18 for details")
